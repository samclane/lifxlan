...
def main():
    ...
    try:
        start_time = time()
        while True:
            color = list(original_colors.values())[0]
            dim = list(copy(color))
            dim[2] = 1900
            g.set_color(dim, half_period_ms, rapid=True)
            sleep(half_period_ms/1000.0)

            # color = original_colors[0]
            g.set_color(color, half_period_ms, rapid=True)
            sleep(half_period_ms/1000.0)
